...
const DEPTH_INDENT = ["pl-0", "pl-3 sm:pl-4", "pl-6 sm:pl-8", "pl-9 sm:pl-12"] as const;

type MobileTreeEntry =
  | { kind: "node"; id: OrgNodeId; depth: number; dashed?: boolean }
  | { kind: "note"; key: "organogramAdminLine" | "organogramIndependence"; depth: number };

/** Vertical hierarchy for small screens — mirrors the official chart order. */
const MOBILE_TREE: MobileTreeEntry[] = [
  { kind: "node", id: "board", depth: 0 },
  { kind: "node", id: "audit", depth: 1 },
  { kind: "node", id: "complianceEthics", depth: 1 },
  { kind: "node", id: "remuneration", depth: 1 },
  { kind: "node", id: "ceo", depth: 1 },
  { kind: "note", key: "organogramAdminLine", depth: 2 },
  { kind: "node", id: "cro", depth: 2 },
  { kind: "node", id: "investment", depth: 3 },
  { kind: "node", id: "transaction", depth: 3 },
  { kind: "node", id: "rm", depth: 2 },
  { kind: "node", id: "ccs", depth: 2 },
  { kind: "node", id: "infoSystem", depth: 3 },
  { kind: "node", id: "hr", depth: 3 },
  { kind: "node", id: "legal", depth: 3 },
  { kind: "node", id: "accounting", depth: 3 },
  { kind: "node", id: "cco", depth: 1, dashed: true },
  { kind: "note", key: "organogramIndependence", depth: 2 },
];

function OrganogramMobileTree({
  boxProps,
  t,
}: {
  boxProps: (id: OrgNodeId) => OrgBoxProps;
  t: ReturnType<typeof useI18n>["t"];
}) {
  return (
    <div className="space-y-2 lg:hidden">
      {MOBILE_TREE.map((entry, index) => {
        if (entry.kind === "note") {
          return (
            <p
              key={`${entry.key}-${index}`}
              className={cn(
                DEPTH_INDENT[entry.depth] ?? DEPTH_INDENT[3],
                "py-1 text-[9px] uppercase leading-snug tracking-[0.1em] text-slate-warm",
              )}
            >
              {t.about[entry.key]}
            </p>
        ...
        const depthClass = DEPTH_INDENT[entry.depth] ?? DEPTH_INDENT[3];

        return (
          <div
            key={entry.id}
            className={cn(depthClass, entry.depth > 0 && "border-l border-hairline")}
          >
            <OrgBox
              {...boxProps(entry.id)}
              compact={entry.depth > 0}
              dashed={entry.dashed}
            />
          </div>
        );
...
function OrganogramDesktopChart({
  boxProps,
  isLit,
  dimming,
  hoveredId,
  ...
  boxProps: (id: OrgNodeId) => OrgBoxProps;
  isLit: (id: OrgNodeId) => boolean;
  dimming: boolean;
  hoveredId: OrgNodeId | null;
  t: ReturnType<typeof useI18n>["t"];
}) {
  return (
    <div className="hidden overflow-x-auto pb-2 lg:block">
      <div className="mx-auto min-w-[58rem] px-2">
        ...
        <div className="mx-auto flex h-8 justify-center">
          <VRail
            className="h-full"
            lit={isLit("board") && (isLit("ceo") || isLit("cco") || isLit("audit"))}
          />
        </div>
        ...
        <div className="relative grid grid-cols-[14rem_1fr_14rem] gap-4">
          ...
          <div className="absolute left-[7rem] top-0 h-6">
            <VRail
              className="h-full"
              lit={isLit("audit") || isLit("complianceEthics") || isLit("remuneration")}
            />
          </div>
          ...
          <div className="flex flex-col items-center pt-6">
            ...
            <div className="flex h-8 justify-center">
              <VRail
                className="h-full"
                lit={isLit("ceo") && (isLit("cro") || isLit("rm") || isLit("ccs"))}
              />
            </div>
              ...
              <div className="flex flex-col items-center gap-2 pt-5">
                ...
                <div className="flex h-5 justify-center">
                  <VRail
                    className="h-full"
                    lit={
                      isLit("infoSystem") || isLit("hr") || isLit("legal") || isLit("accounting")
                    }
                  />
                </div>
                <div className="grid w-full gap-2">
...
export function Organogram() {
  ...
  const boxProps = (id: OrgNodeId): OrgBoxProps => ({
    id,
    active: activeId === id,
  ...
  const mobileBoxProps = (id: OrgNodeId): OrgBoxProps => ({
    id,
    active: activeId === id,
    hovered: false,
    dimmed: false,
  ...
  return (
    <div className="mt-12">
      ...
      <OrganogramMobileTree boxProps={mobileBoxProps} t={t} />
      <OrganogramDesktopChart
        boxProps={boxProps}
        isLit={isLit}
        dimming={dimming}
        hoveredId={hoveredId}
      ...
      {/* Hover preview strip — desktop only */}
      {hoveredId && hoveredId !== activeId && (
        <div className="mt-8 hidden border border-hairline bg-panel px-5 py-4 transition-all duration-300 animate-in fade-in-0 slide-in-from-bottom-1 lg:block">
      ...
      <div
        key={active.id}
        className="mt-8 animate-in fade-in-0 slide-in-from-bottom-2 border-t border-hairline duration-500 fill-mode-both lg:mt-12"
      >
        <div className="grid gap-8 py-8 md:grid-cols-12 md:py-10">
          <div className="md:col-span-4">
            <div className="eyebrow">{active.subtitle}</div>
            <h3 className="mt-3 font-serif text-2xl text-navy md:text-3xl">{active.label}</h3>
