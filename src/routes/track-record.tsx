...
export const Route = createFileRoute("/track-record")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "A curated ledger of BluFin's advisory transactions across corporate finance, listings, M&A, and private equity." },
...
const SECTORS = ["All", "Retail", "Logistics", "Tech", "Manufacturing", "Financial Services", "Energy"] as const;
...
function TrackRecord() {
  ...
  const filtered = useMemo(() => {
    return txns.filter((t) => {
      ...
      if (q && !`${t.client} ${t.summary} ${t.service}`.toLowerCase().includes(q.toLowerCase())) return false;
      return true;
    });
  }, [txns, sector, scale, q]);
              ...
              {filtered.map((t, i) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs uppercase tracking-widest text-slate-warm">{t.year}</span>
                  </div>
                  <h3 className="mt-6 font-serif text-xl text-navy">{t.client}</h3>
                  <div className="mt-2 flex flex-wrap gap-2">
                    <span className="border border-hairline px-2 py-1 text-[10px] uppercase tracking-widest text-slate-warm">{t.sector}</span>
                    <span className="border border-hairline px-2 py-1 text-[10px] uppercase tracking-widest text-slate-warm">{t.scale}</span>
                  </div>
